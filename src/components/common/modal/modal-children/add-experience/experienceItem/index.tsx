...
export const ListItem: FC<IExperienceDutyItem> = ({
  ...
  const isCurrentBeingUpdated = updatedItem === id // Comment - а почему это не стейт, может путь все изменения варяться внутри этого компонента?)

  // Comment - какая то мудреная типизация))) - попробуй типизировать евент, без деструкта, а вообще -к огда ты передаешь тупо велью - то его и передавац сразу)
  ...
  // Comment - есил уже используешь такой кондишнл рендеринг - то делай его в верстке. оно будет там нативно понятнее, ты же не свич-кейс модуль на несколько элементов пишешь
